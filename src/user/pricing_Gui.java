...
class New_dbconnect_2{		
	public New_dbconnect_2(String sql) {
		try {
			Class.forName("oracle.jdbc.driver.OracleDriver");
...
class setting_Gui extends JFrame{//처음 세팅 창 화면 설정 
	...
	class del_bt_act implements ActionListener{//del_bt버튼 액션리스너 
		...
		public void actionPerformed(ActionEvent e) {
			//내가 원하는 위치를 가져와 
			// int cl =set_list.getSelectionModel().getLeadSelectionIndex();
			...
			if(cl!=-1) {
				...
				// model.removeRow(set_list.getSelectionModel().getLeadSelectionIndex());
	...
	class repricingSet_Gui {//요굼 수정
		...
		class Change_in_bt implements ActionListener{
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(cchk()) {
					if(m.find()) {
						if(hour_set.getText().isEmpty() && fee_set.getText().isEmpty()) {
						...
						if(m.find()) {
							...
							//System.out.println(hour_set.getText());
							//System.out.println(fee_set.getText());
							//System.out.println(min_box.getSelectedItem());
							...
							//	System.out.println("feeinfo?? :"+Feeinfodata);
	...
	public setting_Gui() {
		...
		set_listSP.setBounds(30,50, 300, 300);
		//set_list.setAutoCreateRowSorter(true);//table 정렬 기능 
		add(set_listSP);
	...
	class pricingSet_Gui {//요금 등록
		...
		class settingact implements ActionListener{
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (cchk()) {
					if(fee_set.getText().isEmpty() && hour_set.getText().isEmpty()) {
						if (m.find()) {
							...
							if(m.find()) {
								if(!hour_set.getText().equals("") && fee >= 0) {
									...
									System.out.println("등록된 요금 : "+Feeinfodata);
